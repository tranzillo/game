...
- **Black erases the past.** Black-color cards that target The Past to *remove* entries (denying Blue's copy/recall plays). The Past is currently write-only — both sides append on every action resolution, never remove. Black will get verbs/effects that can selectively delete entries. Not implemented; noted on 2026-05-22.
- **Recall (Blue keyword).** "Recall: trigger an action from the past, ignoring its requirements." Premium effect — directly resolves a Past action without paying its Insight cost. Reserved for high-cost Blue cards. Not implemented; defined on 2026-05-22.
...
- **The Past:** vertical strip on the left of the encounter UI. Shared encounter-scoped log of action resolutions; every `resolveAction` and quest reward firing appends `{ defKey, side, loc, name, tempo, turn }` to `state.past`. Cleared at encounter start. Blue cards (currently `b10 Magus of Echoes`) target Past entries positionally or randomly. Write-only today; Black erase-the-past is a planned mechanic.
- **Phase 1 of the visual sync layer (timeline + outcomes substrate):** Added `state.timeline` (unified Future/Past chip stream) and `state.outcomes` (narrative events). Helpers `emitFutureChip(card, side, loc, pos, tempo)`, `resolveChipForCard(card, side, loc, pos)`, `emitOutcome(kind, payload)`. Wired into: commit (commits → future chips), encounter-start AI/neutral placements (face-down → future chips), token creation (Keeper golems, Explosive Traps, Quest tokens → resolved chips), reveal-queue passes (chips future → past), action resolution (resolved chip + appendToPast), combat (attack outcome events), damage/death/movement/mark application/mark-tear outcomes. Debug strip below log shows both streams for verification.
- **Phase 2a of the visual sync layer (static unified timeline UI):** Replaced the separate Initiative tracker and Past strip with a single L-shape (Present node at top-left elbow, Future bar marching horizontally to the right with high-Tempo chips closest to the present, Past column hanging down the entire left side of the battlefield alongside the board). Chips show face-up for owning side / face-down (`?`) for opponent, with marks visible on both (per "marks leak through fog by design"). Pending player commits render as pseudo-chips in the future bar so the player can preview Tempo position before advancing. Combat log reframed as narrative history with phase-boundary dividers and icons (⚔ combat, ↑ flip, → action, ⚡ summoner damage, ✕ death). Driven by `state.timeline` directly. Animation (chips marching through present in tempo order) is Phase 2b; persistent chip DOM is Phase 2c.
- **Layout pass:** Unified play area (`#play-area`) contains timeline + battlefield + narrative log as one visual unit. AI summoner bar (`#ai-summoner`) only shown at boss encounters (the only encounter type with a real opposing summoner). Player summoner bar sits below the play area. Player-bottom row contains: Draw pile | Hand | Discard pile | GY pile | JY pile | Exile pile, with hand expanding as the center column.
- **Piles UI:** Each pile renders as a small offset stack of up to 3 card-shaped mini-icons with the top card showing its name (or `?` for the face-down deck), marks visible on the top card if any, and a count below. Face-down minis use a striped pattern to suggest a card back.
- **Phase 2c-step-1 (persistent card DOM registry):** `_cardRegistry: Map<instId, HTMLElement>` keyed by card instId. `getCardOuterEl(card)` returns a stable outer element across renders. `makeCardEl` now consults the registry instead of always creating fresh DOM. `sweepCardRegistry()` runs at end of each render to remove elements whose cards no longer exist in any zone; registry also cleared at encounter start.
- **Phase 2c-step-2 (FLIP animation between zones):** Render is wrapped: `capturePreRenderRects()` captures each registered card's `getBoundingClientRect` before the DOM mutates, then `applyFlipAnimations(preRects)` runs after — computes (old - new) deltas, applies inverse `transform: translate(dx, dy)`, forces reflow, transitions to identity over `FLIP_DURATION_MS` (280ms). Cards visibly fly between hand / slots / piles / pending whenever they change zone or reorder. Newly created cards skip animation (no prior rect); deleted cards just disappear via the sweep.
- **Phase 2c-step-3 polish — flip-up + mark-pop:** `_lastRevealed` tracks each card's previous `revealed` state; AI cards transitioning false→true get a transient `flipping-up` class triggering a CSS `rotateY` flip-in animation. `_lastMarkCount` tracks mark count; newly-appeared mark badges get a `mark-new` class with a pop-in animation.
- **Phase 2b — combat sequencing:** `runCombat(onDone)` is now async-callback. The attack queue is flattened to a sequence with location and side headers, then walked one beat at a time via `setTimeout` chain — each swing renders + waits `COMBAT_STEP_MS` (450ms) before the next. The player sees swings unfold in Tempo order with card-movement and death animations between each. Caller in `resolveCombatPhase` was updated to pass an onDone callback that runs the cleanup transition.
- **Phase 2b — reveal queue sequencing:** `endOfPhaseRevealAndResolve(onDone)` is async-callback. The reveal queue is built+sorted as before, then walked one event at a time via `setTimeout` chain — each event (flip / action / equipment) renders + waits `REVEAL_STEP_MS` (380ms) before the next. The player watches chips march through the present in Tempo order. All five callers updated to pass onDone: `runUpkeepEnd`, `advancePhase` upkeep + draw branches, `endMainPhase`, `resolveCombatPhase`, `endCleanupPhase`. Together with combat sequencing, every per-phase resolution is now visually paced.
...
- **Champion's Rest** (peace location text, key `locP1`): implemented and deployed at A2. Uses suppress framework: location text declares `shouldSuppressAction(card, loc)` returning true unless exactly one creature is here. The action's chip stays in the future bar (suppressed) until the condition is met at any end-of-phase pass, then resolves through the normal queue. No special force-flip path.
