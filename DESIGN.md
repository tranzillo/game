...
- **Structures (transient utility):** Green's structures lean toward *movement and impermanence* rather than permanent infrastructure. Working sketch — **Caravan**: a Green effect that packs up a structure you control, moving it from play into your discard pile, so you can replay the structure at a different location on a future turn. Contrasts with the supply-line model that makes structures permanent at their location.
...
### Encounter time limit at neutrals
...
Principles that emerged from iterating on neutral encounter sketches:

- **Couple temporary effects to permanent outcomes.** Damage, durability, in-encounter buffs and debuffs all reset when cards leave play, so they have no run-impact alone. In-encounter healing matters when it triggers a permanent outcome — overheal firing Convert, damage clearing creatures so a permanent buff lands on a survivor, etc. Healing or buffing in isolation is design-irrelevant at a neutral location. *Permanent* outcomes that matter: card acquisitions, deck-thin removals, permanent stat upgrades on specific cards, permanent location modifications, summoner-Durability changes.
- **Timing is load-bearing.** A heal at upkeep whiffs because the player has nothing on board yet. Triggers must fire when the relevant board state exists — typically end of combat or end of cleanup, after the player has committed in main and combat has resolved.
- **Conflicting trigger conditions create the choice space.** Independent additive effects (commit a creature, get all the buffs) produce flat encounters that read as menus. Effects whose conditions conflict — Effect A requires X alive, Effect B requires X dead, or "this triggers only if exactly one creature is here" — force real decisions. Same cards with tighter conditions can completely reshape an encounter without adding more elements.
- **Phase boundaries are real design surfaces.** End-of-combat and end-of-cleanup are one phase apart but represent meaningfully different decision moments. Placing triggers at specific phase boundaries shapes the player's choices across the turn.
- **Neutrals can attack the player.** The "no run-loss" property comes from per-encounter time limits and shallow neutral resources, not from making neutrals docile. Aggressive neutrals are legal and often on-flavor.
...
- **Smith's Workshop (Red, mountain/Force)** — *Permanent stat-upgrade combo with conflicting triggers.* The encounter places an *Apprentice* creature on the other side (low Force, attacks normally) and a *Forge* structure on the other side. Apprentice: *"At end of combat, give a creature on the other side from this card here +1 Durability permanently."* Forge: *"At end of cleanup, if only one creature is here, it gains +1 Force permanently."* The two triggers conflict — Apprentice's trigger needs it alive at end of combat to fire (so leaving it alive gives the Durability buff); Forge's trigger needs only one creature at the location at end of cleanup (the Apprentice itself is a creature, so leaving it alive prevents Forge from firing). Player options: leave Apprentice alive → +1 Durability only; kill Apprentice in combat → +1 Force only (Forge sees one creature, the player's); thread the needle to get both → narrow window requiring a way to remove Apprentice during cleanup *after* its end-of-combat trigger has fired (rare, aspirational).
- **Forest Glade (Green, forest/Tempo)** — *Reroute deck-thin combo with blocking tension.* Setup on the other side: two generic Force-2 attackers in the front row, plus a *Trail Marker* (structure, Inert): *"At end of turn, opposing creatures here gain a Reroute mark."* Since the Marker is on the other side, the mark redirects player creatures' leave-play destination to the other side's pile — at a neutral encounter where the other side has no deck to reshuffle into, this is a permanent deck-thin. The mark fires *post-combat*, so creatures that died in combat aren't "here" to be marked — chump-blocking with low-Durability chaff is a free defense (chaff dies before the mark applies, no thinning, returns to deck normally). Creatures that *survive* combat get marked and are deck-thinned. Player options: walk away (eat ~4 summoner damage); chump-block with chaff (cheap defense, no thinning, no run-cost); commit a high-Durability blocker that survives combat (defends successfully but is deck-thinned); take summoner damage rather than risk losing valuable cards (forced choice when only high-value blockers are in hand). The encounter *forces* deck-thin onto valuable targets — small chaff cannot be thinned here, which makes the deck-thin a real cost rather than a cheap chaff-cycle.
- **Cursed Ground (Black, swamp/Spite)** — *Raise-via-restraint with over-engagement penalty.* Setup on the other side: two generic Force-2 attackers in the front row, plus a "Hungry Crypt" effect (*placeholder — see note below*): *"At end of cleanup, if exactly one creature on your side here died this turn, raise it as a Brute on the other side from this card."* From the Crypt's POV, "your side" is the other side from the player (where the attackers are); "the other side" is the player's side. So the Crypt raises a dead attacker as a Brute on the player's side — but *only* if exactly one died. Player options: walk away (eat ~4 summoner damage); chump-block (chumps die, attackers survive — no raise, no damage); kill exactly one attacker (raise opportunity, eat damage from the surviving attacker on the unblocked column); kill both attackers (clean defense but the over-engagement penalty loses the raise); Stealswap an attacker (premium acquisition, sacrifice your swap card). Condition tension: the "exactly one" rule punishes over-engagement. The player must *restrain* from clearing the board to keep the raise opportunity — Black-flavored: engagement is the resource, but restraint is the discipline. **Note:** the Hungry Crypt effect is sketched as a structure here as a placeholder. It wouldn't print well as a player card (the conditional auto-raise is too encounter-specific to want in a deck — see DESIGN_LESSONS.md principle 9). Final form is pending the war/peace location-text framework — the effect should live as the location's peace-mode text, not as a structure card.
...
- *Dual-state location text (war / peace mode).* Big-idea exploration: each location prints two rules-text states, one for "peace" (no opposing summoner presence at the overworld node) and one for "war" (opposing summoner presence). Peace text skews toward passive / cooperative outcomes that fit neutral-encounter framing; war text skews toward combat / confrontational. Mixed encounters (a peace-mode neutral location adjacent to a war-mode hostile location in the same encounter) would create unusual interactions where confrontation can still occur at peace locations under peace rules. The idea is interesting but expensive in design work (2x location text per location); held for evaluation.
